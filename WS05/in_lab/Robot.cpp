...
namespace sdds {
	...
	Robot::~Robot() {
		delete [] nickname; // char arrays for example 
		// nickname; // deletes a single char
	}
	...
	Robot::Robot(const char* x_nickname, int x_force, int x_durability):arms(x_force) {
		...
		if (x_nickname == nullptr || strcmp(x_nickname, "") == 0) {
			...
			nickname = new char [strlen(x_nickname) + 1];
	...
	ostream& Robot::display() const{
		if (nickname == nullptr || nickname[0] == '\0'  || durability == 0) {
