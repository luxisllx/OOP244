...
namespace sdds {
	...
	Flower::Flower(const char* name, const char* color, double price) {
		...
		if (name == nullptr || strcmp(name, "") == 0 || color == nullptr || strcmp(color, "") == 0 || price <= 0) {
	...
	void Flower::setName(const char* name, int len) {
		...
		if (f_name != nullptr) {		// if there is a flower name existing, delete it
			delete[] f_name;
		}
		f_name = new char[len + 1];		// allocate memory for flower name
		strcpy(f_name, name);					
								
	}

	void Flower::setColour(const char* colour, int len) {
		if (f_colour != nullptr) {				// if there is existing flower colour, delete it
			delete[] f_colour;
		}

		f_colour = new char[len + 1];	// if there is not an existing flower color, dynamically allocate memory to hold one
	...
	void Flower::setEmpty() {
		if (f_name != nullptr) {			// before setting empty, we need to confirm it's not empty. 
